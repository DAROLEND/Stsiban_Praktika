...
namespace AspNetIdentityApp.Controllers
{
    public class FlowerController : Controller
    {
        ...
        // GET: Flower
        public ActionResult Index(int? supplierId, int page = 1)
        {
            const int pageSize = 2;

            var query = db.Flowers
            ...
            var flowersPaged = query
                               .OrderBy(f => f.Name)
                               .ToPagedList(page, pageSize);

            var suppliers = db.Suppliers.ToList();
            suppliers.Insert(0, new Supplier { Id = 0, Name = "Усі постачальники" });

            var model = new FlowerListViewModel
            {
                Flowers = flowersPaged,
